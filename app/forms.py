...
from app.counterparties import get_types, get_service_types, \
    get_channel_width, get_managers
...
class EditCounterpartiesForm(FlaskForm):
    ...
    vlan_address_from = TextAreaField(
        label='Адес VLAN от',
        render_kw={
            'class': 'form-control',
            'rows': 2,
    ...
    information_source = TextAreaField(
        label='От куда узнали об услугах',
        render_kw={
            'class': 'form-control',
            'rows': 2
        }
    )
    responsible_manager = SelectField(
